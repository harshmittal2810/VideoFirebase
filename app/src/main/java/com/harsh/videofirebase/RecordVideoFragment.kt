...
import com.harsh.videofirebase.utils.*
import java.io.File
...
class RecordVideoFragment : Fragment(), View.OnClickListener,
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(cameraDevice: CameraDevice) {
            ...
            this@RecordVideoFragment.cameraDevice = cameraDevice
            startPreview()
        ...
        override fun onDisconnected(cameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            cameraDevice.close()
            this@RecordVideoFragment.cameraDevice = null
        }
...
//        uploadFromFile(path)
    ...
    private fun initFirebase() {
        ...
        folderRef = storageRef.child("videos")
...
//            }
    ...
    private fun stopRecordingVideo() {
        ...
        if (activity != null) {
            ...
            uploadFromFile(nextVideoAbsolutePath ?: "")
        }
    ...
    companion object {
        fun newInstance(): RecordVideoFragment = RecordVideoFragment()
    }
    ...
    private fun uploadFromFile(path: String) {
        ...
        fileURI?.let { fileURI ->
            val metadata = StorageMetadata.Builder()
                .setCustomMetadata("country", "India")
                .setContentType("video/mp4")
                .build()
            val videoRef: StorageReference = folderRef.child(fileURI.lastPathSegment ?: "")
            mUploadTask = videoRef.putFile(fileURI, metadata)
            Helper.initProgressDialog(requireContext(), "Uploading")
            Helper.mProgressDialog.setButton(
                DialogInterface.BUTTON_NEGATIVE,
                "Cancel"
            ) { _, _ -> mUploadTask.cancel() }
            Helper.mProgressDialog.show()
            mUploadTask.addOnFailureListener { exception ->
                Helper.dismissProgressDialog()
                Toast.makeText(
                    requireContext(),
                    String.format("Failure: %s", exception.message),
                    LENGTH_SHORT
                ).show()
                ...
                videoRef.downloadUrl.addOnSuccessListener { uri ->
                    uri?.let {
                        ...
                        val uploadId =
                            mDatabaseRef.push().key
                                ?: kotlin.run { "${System.currentTimeMillis()}" }
